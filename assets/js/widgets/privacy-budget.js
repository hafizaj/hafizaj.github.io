...
(function () {
  ...
  window.PrivacyBudget = window.PrivacyBudget || {};
  window.PrivacyBudget.noiseStd = function (k, epsilonTotal, sensitivity) {
  ...
  function mount(el) {
    ...
    var yMax = window.PrivacyBudget.noiseStd(KMAX, EPS_TOTAL, DELTA) * 1.05;
    ...
    function draw() {
      ...
      for (var kv = 1; kv <= KMAX; kv += 0.5) {
        var x = px(kv), y = py(window.PrivacyBudget.noiseStd(kv, EPS_TOTAL, DELTA));
        if (kv === 1) ctx.moveTo(x, y); else ctx.lineTo(x, y);
      }
      ctx.stroke();

      var mx = px(k), my = py(window.PrivacyBudget.noiseStd(k, EPS_TOTAL, DELTA));
      ctx.fillStyle = ROYAL;
      ctx.beginPath(); ctx.arc(mx, my, 4.5, 0, Math.PI * 2); ctx.fill();

      if (readout) {
        readout.textContent = 'k = ' + k + '   per-query ε = ' + (EPS_TOTAL / k).toFixed(3) +
          '   noise std = ' + window.PrivacyBudget.noiseStd(k, EPS_TOTAL, DELTA).toFixed(2);
      }
    }
